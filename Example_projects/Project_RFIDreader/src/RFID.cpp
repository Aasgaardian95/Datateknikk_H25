...
void loop() {
  ...
  for (byte i = 0; i < uidLen; i++) {
    if (uid[i] < 0x10) Serial.print('0'); // Legger til ledende null hvis tallet er under 0x10
    Serial.print(uid[i], HEX);            // Skriver ut byte i hex-format
    if (i < uidLen - 1) Serial.print(':'); // Setter kolon mellom bytes
  }
  Serial.println();
...
bool isAllowed(const byte *uid, byte len) {
  ...
  for (int i=0;i<ALLOWED_COUNT;i++) {
    if (len != ALLOWED_LEN[i]) continue; // Hopper over hvis lengden ikke stemmer
    bool match = true;
    // Sammenligner byte for byte
    for (byte j=0;j<len;j++) {
      if (uid[j] != ALLOWED[i][j]) { match=false; break; }
    }
    if (match) return true; // Returnerer true hvis alle bytes stemmer
  }
