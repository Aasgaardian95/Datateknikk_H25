...
enum State {GREEN, YELLOW, RED, EMERGENCY};
State state = GREEN;
...
void loop() {
  ...
  switch(state) {
    case GREEN:
      ...
      if (millis() - lastChange > 5000) {state = YELLOW; lastChange = millis();}
      break;
    case YELLOW:
      digitalWrite(green, LOW);
      digitalWrite(yellow, HIGH);
      digitalWrite(red, LOW);
      if (millis() - lastChange > 2000) {state = RED; lastChange = millis();}
      break;
    case RED:
      digitalWrite(green, LOW);
      digitalWrite(yellow, LOW);
      digitalWrite(red, HIGH);
      if (millis() - lastChange > 5000) {state = GREEN; lastChange = millis();}
      break;
    case EMERGENCY:
      digitalWrite(green, LOW);
