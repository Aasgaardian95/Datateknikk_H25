...
void setup() {
  ...
  // --- Initialiser OLED ---
  if(!display.begin(SSD1306_SWITCHCAPVCC, 0x3C)) { // I2C 0x3C
...
void loop() {
  ...
  if (!override) {
    if (hum < 40 && tvoc < 350 && eco2 < 1000) { // Lav fuktighet og god luft
      pumpSpeed = 255;                           // Full hastighet
    } else {                                     // Ellers stopp pumpe
      pumpSpeed = 0;
    }
  } else {                                       // Overstyring aktiv
    pumpSpeed = 255;                             // Full hastighet
  }
