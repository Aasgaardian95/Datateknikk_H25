...
void loop() {
  ...
  for(int i = 0; i < NUM_PIXELS; i++){
    if(i < ledsToLight){
      if(distance > 50) strip.setPixelColor(i, strip.Color(0,255,0));      // Grønn lang avstand
      else if(distance > 20) strip.setPixelColor(i, strip.Color(255,255,0)); // Gul medium avstand
      else strip.setPixelColor(i, strip.Color(255,0,0));                     // Rød kort avstand
    } else {
